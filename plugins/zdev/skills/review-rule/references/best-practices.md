...
ルールファイルに記載すべき内容かどうかを判断する5基準。新規ルール追加時・レビュー時に照合する。

### 5基準チェックリスト

- [ ] **非自明性**: コードを読めば推測できる規約ではないか。コードから推測可能なら不要
- [ ] **違反影響度**: 違反時にアーキテクチャ崩壊・データ不整合・セキュリティ問題が起きるか。軽微な影響なら不要
- [ ] **判断分岐の頻度**: 開発中にその判断に直面する頻度が高いか。稀にしか発生しない判断なら不要
- [ ] **自動検証の補完**: ESLint・型チェック・dependency-cruiser 等で検出できない規約か。自動検出可能なら不要
- [ ] **コンテキストコスト**: ルール追加によるコンテキストウィンドウ消費に見合う価値があるか
...
|--------|------|
| 4〜5個 該当 | ルール化すべき |
| 2〜3個 該当 | 既存ルールへの追記で対応 |
| 0〜1個 該当 | ルール化不要 |

### アンチパターン: 不要なルール化

以下はルール化すべきでない典型例:
- コードに一貫したパターンがあり、読めば自明な規約（例: エラーハンドリング戦略）
- 自動検証ツールが既にカバーしている規約（例: ESLint ルールで検出可能な書式）
- 発生頻度が極めて低い判断（例: 年に一度の設計選択）
- コンテキスト消費に対して効果が薄い長文の説明
