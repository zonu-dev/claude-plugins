---
description: "エージェント定義ファイルを作成し、レビュー・自動修正まで行う。「エージェントを作成」「新しいエージェントを作って」「エージェント作成して」などのリクエストで起動します。"
...
AskUserQuestionツールでエージェント種別を確認:
- 質問: 「エージェントの種別を選択してください」
- 選択肢:
  - **分析・調査（推奨）**: コードベースの分析・調査を行う。ツール: `Glob, Grep, Read`
  - **実装・修正**: コードの実装・修正を行う。ツール: `Glob, Grep, Read, Edit, Write, Bash`
  - **レビュー**: コードやドキュメントのレビューを行う。ツール: `Glob, Grep, Read`
  - **ドキュメント生成**: ドキュメントの生成・更新を行う。ツール: `Glob, Grep, Read, Edit, Write`

### モデルの選択

AskUserQuestionツールでモデルを確認:
- 質問: 「使用するモデルを選択してください」
- 選択肢:
  - **sonnet（推奨）**: バランスの良い汎用モデル。大半のタスクに適切。
  - **haiku**: 高速・低コスト。単純な分類・抽出・定型処理向け。
  - **opus**: 最高性能。高度な推論・複雑な判断が必要なタスク向け。
