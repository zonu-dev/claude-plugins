---
description: GitHub PRのコードレビューを行う。PRの差分を分析し、コード品質・セキュリティ・パフォーマンス・ベストプラクティスの観点から構造化されたレビューを提供し、gh CLIでレビューアクションを実行する。
...
## 総合評価
<Approve / Request Changes / Comment の推奨とその理由>
```
...
レビューレポート提示後、ユーザーに次のアクションを確認する：
1. **Approve**: PRを承認する
2. **Request Changes**: 変更を要求する
3. **Comment**: コメントのみ残す
4. **何もしない**: レポートの確認のみ

ユーザーが1-3を選択した場合、`gh pr review` コマンドで実行する：
```bash
...
実行後、投稿が成功したことを確認する：
```bash
gh pr view <PR番号> --json reviews --jq '.reviews[-1]'
```

失敗時はエラー内容を表示し、リトライオプションを提示する。
